...
export default {
  ...
  mounted() {
    this.$nextTick(async () => {
      try {
        await Promise.all([
          this.GET_DATA(this.apiUrl+'/'+this.$route.params.id),this.GET_DATA("colaboradores"),this.GET_DATA("resorts"),this.GET_DATA("areas"),this.GET_DATA("equipos")
          ]).then((v) => {
          this.model = v[0];

          this.colaboradores = v[1];
          this.resorts = v[2];
          this.areas = v[3];
          this.equipos = v[4];
