              ...
              <CrudUpdate :model="model" :apiUrl="apiUrl">
                <div slot="body" class="row">
                  <div class="form-group col-2">
                    <label for="">Número de colaborador</label>
                    <input
                      type="text"
                      name=""
                  ...
                  <div class="form-group col-2">
                    ...
                    <select v-model="model.estado" class="form-control">
                      <option value="habilitado" style="color: green;">Habilitado</option>
                      <option value="deshabilitado" style="color: red;">Deshabilitado</option>
                    </select>
                  </div>
...
export default {
  ...
  mounted() {
    this.$nextTick(async () => {
      try {
        await Promise.all([
          this.GET_DATA(this.apiUrl+'/'+this.$route.params.id),this.GET_DATA("areas"), this.GET_DATA("resorts")
        ]).then((v) => {
          this.model = v[0];
          this.areas = v[1];
          this.resorts = v[2];
